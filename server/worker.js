...
async function init() {
    ...
    while (waitingBroadcastRes) {
        ...
        if (broadcastResponse.length >= workersNodes.length) { init(); }
...
async function convertPS2PDF(fileName, originalName) {
    ...
    let encode = files.rows[0].file.data
